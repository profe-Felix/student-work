...
/* =============================================================================
   REALTIME: GLOBAL BROADCAST (no schema changes)
   - Teacher toggles "Allow colors" -> broadcast setAllowColors
   - Students subscribe via subscribeToGlobal to receive updates live
============================================================================= */

// Subscribe to a global channel for class-wide controls.
// Returns an unsubscribe function.
export function subscribeToGlobal(
  onMessage: (msg: { type: string; payload?: any }) => void
): () => void {
  // self:false so we don't loopback local broadcasts on the same tab (teacher side can choose self:true if needed)
...
export async function setAllowColors(allow: boolean) {
  ...
  await ch.subscribe((status) => {
    if (status === 'SUBSCRIBED') {
      ch.send({
        type: 'broadcast',
        event: 'setAllowColors',
        payload: { allowColors: allow },
      })
      // optional cleanup to avoid lingering channel
      ch.unsubscribe()
    }
  })
}
