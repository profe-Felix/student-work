...
function isPoint(v: unknown): v is StrokePoint { return isRecord(v) && Number.isFinite((v as any).x) && Number.isFinite((v as any).y) }
...
function normalize(input: StrokesPayload | null | undefined): StrokesPayload {
  ...
  const safe: Stroke[] = raw.map((s) => {
    ...
    if (isRecord(s)) {
      ...
      const tool  : Stroke['tool'] = tRaw === 'highlighter' ? 'highlighter' : (tRaw === 'eraser' ? 'eraser' : 'pen')
...
export default forwardRef<DrawCanvasHandle, Props>(function DrawCanvas(
  ...
  useImperativeHandle(ref, () => ({
    ...
    undo: (): void => { strokes.current.pop(); redraw() },
    applyRemote: (u) => {
  ...
  useEffect(()=>{
    ...
    const onPointerDown = (e: PointerEvent)=>{
      ...
      onStrokeUpdate?.({ id: localStrokeId.current!, color, size, tool: t, pts: [p], done: false, from: selfId })
      redraw()
